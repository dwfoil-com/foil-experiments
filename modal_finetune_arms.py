#!/usr/bin/env python3
"""
Fine-tune baseline with arms_full rewards.

Loads ent005_2M checkpoint and continues training with arm rewards.
This is what the exploration actually tested.
"""
...
app = modal.App("pump-foil-finetune-arms")
...
@app.function(image=image, gpu="A10G", timeout=14400, volumes={"/data": baseline_volume})
def finetune_arms(
    timesteps: int,
    baseline_path: str,
    foil_config: dict,
    ...
    """Fine-tune from baseline with arms_full rewards."""
    import sys
    ...
    def create_video(model, pct, output_path, max_frames=500):
        ...
        def animate(frame_num):
            ...
            ax_foil.set_title(f"finetune {pct}% | r={stats['arm_leg_corr']:.2f} | {stats['pump_freq']:.1f}Hz\n"
    ...
    class CheckpointCallback(BaseCallback):
        ...
        def _on_step(self):
            steps_done = self.num_timesteps - self.start_timesteps
            pct = (steps_done / self.total_timesteps) * 100
            for p in self.percentages:
                if p not in self.saved and pct >= p:
                    model_path = os.path.join(self.save_path, f"model_{p}pct")
                    self.model.save(model_path)
                    print(f"\n>>> Checkpoint: {p}% ({steps_done:,} new steps)")
    ...
    print(f"=== FINE-TUNING FROM BASELINE ===")
    print(f"Baseline: {baseline_path}")
    print(f"Arm weights: {arm_weights}")
    print(f"New timesteps: {timesteps:,}")
    print()
    ...
    print(f"Baseline loaded (trained for {model.num_timesteps:,} steps)")

    # Initial evaluation
    print("\nBaseline evaluation:")
    eval_env = PumpFoilEnvExplore(config=foil_config, **arm_weights)
    obs, _ = eval_env.reset(seed=42)
    frames = []
    for step in range(1000):
        action, _ = model.predict(obs, deterministic=True)
        frames.append({
            'left_leg': eval_env.left_leg_pos, 'right_leg': eval_env.right_leg_pos,
            'left_arm': eval_env.left_arm_pos, 'right_arm': eval_env.right_arm_pos,
            'vx': eval_env.state.vx,
        })
        obs, _, terminated, truncated, info = eval_env.step(action)
        if terminated or truncated:
            break
    init_stats = analyze_behavior(eval_env, frames)
    init_duration = len(frames) * eval_env.dt
    print(f"  {init_duration:.1f}s | r={init_stats['arm_leg_corr']:.2f} | "
          f"{init_stats['pump_freq']:.1f}Hz | arm={init_stats['arm_range']:.0f}deg")

    # Fine-tune
    print(f"\nFine-tuning for {timesteps:,} additional steps...")
    ...
    results = {'checkpoints': {}, 'videos': {}, 'stats': callback.stats,
               'init_duration': init_duration, 'init_stats': init_stats}
...
def main(
    ...
    output_dir: str = "checkpoints/finetune_arms_1M",
    baseline: str = "checkpoints/ent005_2M/model_100pct.zip",
):
    """Fine-tune baseline with arms_full rewards."""

    print(f"=== FINE-TUNE ARMS_FULL ===")
    ...
    print(f"\n{'='*80}")
    print("FINE-TUNE ARMS_FULL SUMMARY")
    print(f"{'='*80}")
    print(f"Baseline: {results['init_duration']:.1f}s | r={results['init_stats']['arm_leg_corr']:.2f}")
    print()
    print(f"{'Checkpoint':<12} {'Duration':>10} {'Arm-Leg r':>12} {'Pump Hz':>10} {'Arm deg':>10}")
    print("-"*80)
    ...
    for pct in sorted(stats.keys()):
        ...
        print(f"{pct}%{'':<10} {s['duration']:>10.1f}s {s['arm_leg_corr']:>12.2f} "
              f"{s['pump_freq']:>10.1f} {s['arm_range']:>10.0f}")

    # Show improvement
    if 100 in stats:
        final = stats[100]
        improvement = final['duration'] - results['init_duration']
        print(f"\nImprovement: {improvement:+.1f}s ({results['init_duration']:.1f}s → {final['duration']:.1f}s)")
