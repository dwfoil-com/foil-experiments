...
# Training foil config (must match training)
FOIL_CONFIG = {
    'S_stab': 0.035,
    'stab_angle': -4.0,
    'S': 0.18,
...
FLIGHT_LIMITS = compute_flight_limits(FOIL_CONFIG)
...
def analyze_pumping(frames, dt=0.01, max_leg_ext=0.15):
    """Analyze pumping behavior from frames."""
...
def create_evolution_video(checkpoint_dir: str, output_path: str = 'training_evolution.mp4'):
    ...
    for pct, path in checkpoints:
        ...
        print(f"  {stats['duration']:.1f}s | range={stats['leg_range_pct']:.0f}% | freq={stats['freq']:.1f}Hz | {term}")
    ...
    def animate(frame_num):
        ...
        for idx, s in enumerate(scenarios[:n_panels]):
            ...
            # Draw water first (background) with velocity-based streaks
            ...
            title_color = 'red' if is_crashed else s['color']
            ax.set_title(f"Training {s['name']}\n{status}", fontsize=10, fontweight='bold', color=title_color)

        # Info panel
        ax_info.clear()
        ax_info.axis('off')
        ax_info.text(0.5, 0.95, "Training Evolution", fontsize=12, fontweight='bold',
                    ha='center', transform=ax_info.transAxes)
        ax_info.text(0.5, 0.85, f"Checkpoints: {[s['name'] for s in scenarios]}", fontsize=8,
                    ha='center', transform=ax_info.transAxes, color='gray')

        for j, s in enumerate(scenarios):
            y = 0.7 - j * 0.15
            ax_info.scatter([0.1], [y], c=[s['color']], s=60, transform=ax_info.transAxes)
            stats = s['stats']
            ax_info.text(0.18, y, f"{s['name']}: {stats['duration']:.1f}s, {stats['leg_range_pct']:.0f}% range, {stats['freq']:.1f}Hz",
        ...
        for ax, ylabel, limits, value_fn in plot_configs:
            ...
            if ax == ax_z:
                ...
                ax.axhline(y=FLIGHT_LIMITS['max_height_cm'], color='red', linestyle='--',
                          linewidth=1, alpha=0.7, label='breach')
                ax.axhline(y=FLIGHT_LIMITS['min_height_cm'], color='red', linestyle='--',
                          linewidth=1, alpha=0.7, label='touchdown')
                ax.axhline(y=0, color='blue', linestyle='-', linewidth=1, alpha=0.5)  # water surface reference
                ax.legend(loc='upper right', fontsize=6)
            elif ax == ax_vx:
                # Stall velocity line
                ax.axhline(y=FLIGHT_LIMITS['stall_velocity'], color='red', linestyle='--',
...
if __name__ == "__main__":
    ...
    create_evolution_video(args.checkpoint_dir, args.output)
