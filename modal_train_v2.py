#!/usr/bin/env python3
"""
Modal GPU training for PumpFoilEnvV2.

Key features:
- Phase encoding in observations
- Jerk penalty for smooth motion
- Simplified reward (5 terms)
- VecNormalize for better training stability
"""
...
def train_v2(
    ...
    from foil_env.pump_foil_env_v2 import PumpFoilEnvV2
    ...
    def analyze_behavior(env, frames):
        ...
        # Jerk
        if len(frames) >= 3:
            actions = np.array([f.get('action', np.zeros(5)) for f in frames])
            if len(actions) >= 3:
    ...
    def create_video(model, vec_normalize, pct, output_path, max_frames=500):
        """Generate video showing model behavior."""
        # Create fresh env for evaluation
        env = PumpFoilEnvV2(config=foil_config)
        obs, _ = env.reset(seed=42)

        frames = []
        for step in range(max_frames):
            # Normalize observation like during training
        ...
        def animate(frame_num):
            ...
            ax_foil.set_title(f"V2 {pct}% | r={stats['arm_leg_corr']:.2f} | {stats['pump_freq']:.1f}Hz\n"
    ...
    class CheckpointCallback(BaseCallback):
        def __init__(self, total_timesteps, save_path, video_path, percentages, vec_normalize):
            super().__init__()
        ...
        def _on_step(self):
            ...
            for p in self.percentages:
                if p not in self.saved and pct >= p:
                    ...
                    try:
                        stats = create_video(self.model, self.vec_normalize, p, video_file)
                        self.stats[p] = stats
    ...
    print(f"=== V2 TRAINING (phase encoding + jerk penalty + simplified reward) ===")
    ...
    callback = CheckpointCallback(timesteps, save_path, video_path, percentages, env)
