...
def draw_rider(ax, foil_positions, theta, left_leg, right_leg, left_arm, right_arm,
    ...
    # Arms - use raw radians directly (no normalization)
    # arm positions are in radians, use them directly as swing angles
    l_arm_rad = left_arm if abs(left_arm) <= max_arm_swing else left_arm * max_arm_swing
    r_arm_rad = right_arm if abs(right_arm) <= max_arm_swing else right_arm * max_arm_swing
