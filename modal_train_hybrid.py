#!/usr/bin/env python3
"""
Modal GPU training for hybrid ACMPC-style pump foil.

Compares:
- Hybrid (MPC-like planner + RL)
- Pure RL baseline

The hybrid approach uses structured trajectory planning for arm-leg coordination
while RL learns the pump parameters.
"""
...
app = modal.App("pump-foil-hybrid")
...
def train_hybrid(
    ...
    n_envs: int = 16,
    ent_coef: float = 0.01,
    control_mode: str = "params_only",
):
    """Train with hybrid ACMPC-style environment."""
    import os
    import sys
    ...
    from foil_env.pump_foil_env_hybrid import PumpFoilEnvHybrid
    ...
    def analyze_coordination(env, frames):
        """Analyze arm-leg coordination."""
        if len(frames) < 50:
            return {'arm_leg_corr': 0, 'pump_freq': 0, 'arm_range': 0, 'leg_range': 0}
    ...
    def create_video(model, pct, output_path, max_frames=500):
        ...
        env = PumpFoilEnvHybrid(config=foil_config, control_mode=control_mode)
        ...
        stats = analyze_coordination(env, frames)
        ...
        def animate(frame_num):
            ...
            ax_foil.set_title(f"Hybrid {pct}% | r={stats['arm_leg_corr']:.2f}\n"
                             f"Arm: {stats['arm_range']:.0f}° Leg: {stats['leg_range']:.0f}cm",
            ...
            ax_plots.set_title(f"{duration:.1f}s | {stats['pump_freq']:.1f}Hz | {term_reason}", fontsize=10)
            return []
                ...
                if p not in self.saved and pct >= p:
                    ...
                    try:
                        ...
                        print(f"    {stats['duration']:.1f}s | r={stats['arm_leg_corr']:.2f} | "
                              f"Arm: {stats['arm_range']:.0f}° | {stats['term_reason']}")
...
def main(
    ...
    output_dir: str = "checkpoints/hybrid",
    ent_coef: float = 0.02,
    control_mode: str = "params_only",
):
    """Train hybrid ACMPC-style model."""
    ...
    print(f"{'='*70}")
    print(f"{'Checkpoint':<12} {'Duration':>10} {'Arm-Leg r':>12} {'Arm Range':>12} {'Pump Hz':>10}")
    print("-"*70)
    ...
    for pct in sorted(stats.keys()):
        ...
        print(f"{pct}%{'':<10} {s['duration']:>10.1f}s {s['arm_leg_corr']:>12.2f} "
              f"{s['arm_range']:>10.0f}° {s['pump_freq']:>10.1f}")
