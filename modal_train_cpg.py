#!/usr/bin/env python3
"""
Modal GPU training for CPG-based pump foil.

Key features:
- CPG action space (4D params instead of 5D velocities)
- Phase encoding in observations
- Jerk penalty for smooth motion
- Optional residual learning on manual baseline
"""
...
def train_cpg(
    ...
    """Train with CPG-based environment."""
    import sys
    ...
    from foil_env.pump_foil_env_cpg import PumpFoilEnvCPG
    ...
    def analyze_cpg_behavior(env, frames):
        """Analyze CPG-based pumping behavior."""
        if len(frames) < 50:
            return {
                'arm_leg_corr': 0, 'pump_freq': 0, 'arm_range': 0, 'leg_range': 0,
                'velocity_loss': 0, 'jerk_mean': 0
            }
        ...
        # Correlation (should be negative for anti-phase)
        if np.std(leg_pos) > 0.001 and np.std(arm_pos) > 0.001:
            corr = np.corrcoef(leg_pos, arm_pos)[0, 1]
        else:
            corr = 0

        # Pump frequency from leg velocity crossings
        ...
        if len(frames) >= 3:
            actions = np.array([f.get('action', np.zeros(4))[:4] for f in frames])
            if len(actions) >= 3:
                jerks = np.abs(actions[2:] - 2*actions[1:-1] + actions[:-2])
    ...
    def create_video(model, pct, output_path, max_frames=500):
        ...
        for step in range(max_frames):
            ...
            frames.append({
                ...
                'waist': env.waist_pos,
                'cpg_freq': env.current_freq,
                'cpg_phase': env.cpg_phase,
                'action': action,
            })
        ...
        stats = analyze_cpg_behavior(env, frames)
        ...
        def animate(frame_num):
            ...
            ax_foil.set_title(f"CPG {pct}% | r={stats['arm_leg_corr']:.2f} | {stats['pump_freq']:.1f}Hz\n"
                             f"Leg: {stats['leg_range']:.0f}cm | Arm: {stats['arm_range']:.0f}°",
            ...
            ax_plots.plot(t_arr, [f['cpg_freq']*10 for f in frames[:i+1]], 'c--', label='Freq x10', lw=1, alpha=0.5)
            ...
            ax_plots.set_title(f"{duration:.1f}s | dV={stats['velocity_loss']:.2f}m/s | jerk={stats['jerk_mean']:.3f} | {term_reason}", fontsize=10)
            return []
        ...
        return {
            ...
            'jerk_mean': float(stats['jerk_mean']),
        }
                ...
                if p not in self.saved and pct >= p:
                    ...
                    try:
                        ...
                        print(f"    {stats['duration']:.1f}s | r={stats['arm_leg_corr']:.2f} | "
                              f"{stats['pump_freq']:.1f}Hz | dV={stats['velocity_loss']:.2f} | {stats['term_reason']}")
    ...
    mode_str = "baseline" if use_baseline else "full"
    residual_str = "+residual" if use_residuals else ""
    print(f"=== CPG TRAINING ({mode_str}{residual_str}) ===")
    ...
    # Use VecNormalize for better training stability
...
def main(
    ...
    output_dir: str = "checkpoints/cpg",
    ent_coef: float = 0.01,
    use_baseline: bool = True,
    use_residuals: bool = False,
):
    """Train CPG-based model."""
    ...
    # Print summary
    print(f"\n{'='*80}")
    print("CPG TRAINING SUMMARY")
    print(f"{'='*80}")
    print(f"{'Checkpoint':<12} {'Duration':>10} {'Arm-Leg r':>12} {'Pump Hz':>10} {'dV (m/s)':>12} {'Jerk':>10}")
    print("-"*80)
    ...
    for pct in sorted(stats.keys()):
        ...
        print(f"{pct}%{'':<10} {s['duration']:>10.1f}s {s['arm_leg_corr']:>12.2f} "
              f"{s['pump_freq']:>10.1f} {s['velocity_loss']:>12.2f} {s['jerk_mean']:>10.3f}")
